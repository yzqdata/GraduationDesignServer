...
<div class="modal fade" id="addOrUpdateModal" tabindex="-1" role="dialog" aria-labelledby="addroleLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="addroleLabel">审批社团</h4>
            </div>
            <div class="modal-body">
                <form id="addOrUpdateForm" class="form-horizontal form-label-left" novalidate>
                    <input type="hidden" name="id">
                    <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="club_name">社团ID: <span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" class="form-control col-md-7 col-xs-12" name="club_id" id="club_id" required="required" placeholder="请输入社团ID"/>
                        </div>
                    </div>
                    <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="club_description">社团名称: <span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" class="form-control col-md-7 col-xs-12" id="club_name" name="club_name" required="required" placeholder="请输入社团名称"/>
                        </div>
                    </div>
                    <@shiro.hasPermission name="resource:community:adminedit">
                        <div class="item form-group">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" class="form-control col-md-7 col-xs-12" name="review_comments" id="review_comments" required="required" placeholder="请输入审核意见"/>
                            </div>
                        </div>
                        <div class="item form-group">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary addOrUpdateBtn">提交注销</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function operateFormatter(code, row, index) {
        ...
        var trUserId = row.id;
        var operateBtn = [
            '<@shiro.hasPermission name="resource:writeoff:update"><a class="btn btn-xs btn-danger btn-update" data-id="' + trUserId + '"><i class="fa fa-trash-o"></i>审批注销</a></@shiro.hasPermission>',
        ];
    ...
    $(function () {
        var options = {
            url: "/writeoff/list",
            getInfoUrl: "/writeoff/get/{id}",
            updateUrl: "/writeoff/edit",
            removeUrl: "/writeoff/remove",
            createUrl: "/writeoff/add",
            saveRolesUrl: "/writeoff/saveUserRoles",
            columns: [
                {
                    checkbox: true
                }, {
